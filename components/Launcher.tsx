  ...
  const handleValidate = async (providedKey?: string) => {
    ...
    try {
      ...
      if (license) {
        ...
        setError('Licence invalide ou serveur injoignable.');
      }
    } catch (e: any) {
      setError('Impossible de se connecter au serveur central.');
    } finally {
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            >
              <div className="flex items-center space-x-2 text-emerald-400 font-black text-[9px] uppercase tracking-widest mb-1">
                <Clock size={12} />
                <span>Utilisateurs Démo</span>
              </div>
              <span className="text-white font-black text-xs uppercase tracking-widest">Démarrer Essai Gratuit</span>
            </button>

            <div className="relative py-2">
              <div className="absolute inset-0 flex items-center"><div className="w-full border-t border-white/5"></div></div>
              <div className="relative flex justify-center"><span className="px-4 bg-transparent text-[8px] font-black text-slate-600 uppercase tracking-widest">Accès Licence</span></div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <button 
                ...
                className="w-full py-4 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl flex items-center justify-center space-x-3 transition-all active:scale-95 disabled:opacity-50"
              >
                {isValidating ? <Loader2 className="animate-spin" size={18} /> : <span>Valider Licence</span>}
              </button>
            </div>
          </div>
        ...
        {showHelp && (
          <div className="bg-emerald-500/10 border border-emerald-500/20 p-5 rounded-3xl animate-in slide-in-from-top-4">
            <div className="flex flex-col items-center text-center text-emerald-400">
              <p className="text-[9px] font-black uppercase tracking-widest mb-1">Service d'assistance</p>

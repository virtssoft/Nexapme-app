...
const SalesHistory: React.FC = () => {
  ...
  const filteredSales = useMemo(() => {
    ...
    // Logic for Week (starting Monday)
    ...
    return sales.filter(sale => {
      ...
      // Filter by Search (Client or ID)
      const matchesSearch = (sale.customerName?.toLowerCase() || '').includes(searchTerm.toLowerCase()) || 
                           sale.id.toLowerCase().includes(searchTerm.toLowerCase());
      
      // Filter by Product inside the sale
      ...
      if (period === 'today') matchesPeriod = saleDate >= startOfToday;
      else if (period === 'week') matchesPeriod = saleDate >= startOfWeek;
      else if (period === 'month') matchesPeriod = saleDate >= startOfMonth;
      ...
      return matchesSearch && matchesPeriod && matchesProduct;
    });
  }, [sales, period, searchTerm, productSearch, startDate, endDate]);
  ...
  const getPaymentBadge = (type: Sale['paymentType']) => {
    switch (type) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 no-print">
        <Branding companyName={config?.name || ''} category="Rapports de Ventes" size="lg" variant="light" />
            ...
            <div className="w-full p-6 flex items-center justify-between">
              <button onClick={() => setExpandedSaleId(expandedSaleId === sale.id ? null : sale.id)} className="flex items-center space-x-5 text-left flex-1">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">
                    ID: {sale.id.toUpperCase()} • {new Date(sale.date).toLocaleDateString()}
              ...
              <div className="flex items-center space-x-8">
                <div className="text-right">
                  ...
                  <p className="text-[9px] text-slate-400 uppercase font-black tracking-widest">{sale.items.length} articles</p>
                </div>
                    ...
                    {sale.items.map((item, idx) => (
                      <div key={idx} className="flex justify-between items-center text-sm">
                        <div className="flex-1">
                          ...
                          <p className="text-[10px] text-slate-400 font-bold">{item.quantity} x {item.unitPrice.toLocaleString()} FC</p>
                        </div>

...
class StorageService {
  ...
  async validateLicenseRemote(key: string): Promise<LicenseInfo | null> {
    try {
      ...
      if (foundPme.status !== 'ACTIVE' && foundPme.license_type !== 'ADMIN') {
        throw new Error("Cette licence est suspendue.");
      }

      const expiry = foundPme.expiry_date;
      if (expiry && new Date(expiry) < new Date() && foundPme.license_type !== 'ADMIN') {
      ...
      if (!isRoot) {
        try {
          ...
          console.warn("Erreur lors de la récupération des utilisateurs de la PME.");
        }
      }
  ...
  async repayCredit(id: string, amount: number) {
    const credits = this.getCredits().map((c: any) => { if (c.id === id) { const rem = Math.max(0, c.remainingAmount - amount); return { ...c, remainingAmount: rem, status: rem === 0 ? 'PAID' : 'PENDING' }; } return c; });
  ...
  async fetchUsers() { 
    ...
    const mapped = data.map(u => ({ id: u.id, name: u.name, role: u.role, pin: '', permissions: typeof u.permissions === 'string' ? JSON.parse(u.permissions) : u.permissions }));
    this.saveUsers(mapped); 
    return mapped; 
  }
  async saveNewUser(u: any) { 
    await ApiService.createUser({ id: u.id || this.generateUUID(), pme_id: this.currentPmeId, name: u.name, role: u.role || 'WORKER', pin_hash: u.pin, permissions: { sales: true, stock: true, history: true } }); 
  ...
  exportAllDataAsJSON() {
    if (!this.currentPmeId) return;
    const data = { company: this.getCompanyInfo(), stock: this.getStock(), sales: this.getSales(), credits: this.getCredits(), cash: this.getCashFlow(), inventory: this.getInventories(), users: this.getUsers(), operations: this.getOperations(), appointments: this.getAppointments(), quotes: this.getQuotes(), subcategories: this.getSubCategories() };
  ...
  importDataFromJSON(jsonText: string) {
    try {
      ...
      if (!this.currentPmeId) { alert("Aucune PME active. Connectez-vous d'abord."); return; }
    ...
    } catch (e) { alert("Erreur lors de l'importation : Format JSON invalide."); }
  }
}
